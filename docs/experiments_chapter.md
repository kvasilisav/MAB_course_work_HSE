...
| E2 | RQ2 | OPE replay (IPS/SNIPS) | OBD random/all | 10 000 соб., 20 seed |
...
| E15 | — | inference, variance reduction | синтетика, 2 руки | 100 trials, T=20 000 |
...
**Дополнение к E1 — сценарии разброса CTR.** Помимо базовой пятирукой среды E1 проверены два крайних случая на той же горизонте T = 5000 и 20 seed: **small_gap** (CTR 4,6–5,4%, шаг ~0,2 п.п.) и **large_gap** (лидер 15% CTR против 1–4% у остальных). При малом разбросе regret у всех политик сходится (21–26), бандит почти не отделяется от `fixed_ab`. При большом разрыве TS резко снижает regret (28,3 против 505,8 у `fixed_ab`): адаптивное перераспределение окупается, когда лучший вариант очевиден (табл. 3.4, рис. 3.4). Для сценариев дополнительно построены bootstrap-ДИ разности regret относительно `fixed_ab` по seed (`synthetic_scenarios --bootstrap 1000`); при **small_gap** интервалы перекрывают 0, при **large_gap** — нет для TS/ε-greedy, что согласуется с визуальным разрывом, но не заменяет полноценный тест на E1.
...
**Дополнение E15 — CUPED на fixed split (синтетика).** Отдельный Monte Carlo на fixed 50/50 A/B с коррелированной доэкспериментной ковариатой (`pre_reward`): 100 trials, T = 20 000, α = 0,05. Сравниваются naïve z-тест и CUPED [7] на скорректированных исходах (табл. 3.1; `outputs/cuped_power/e15_summary.csv`). При null оба метода дают rejection rate 5% (100 trials, номинальный α = 5%). При effect +1 п.п. CUPED даёт мощность 85% против 84% у naïve A/B; средний SE у CUPED ниже (~0,00316 против ~0,00320). При +3 п.п. оба достигают 100% мощности. Это иллюстрация скромного снижения дисперсии на синтетике, а не production-вывод; на OBD ковариаты ограничены (§4.3).

### 3.6.3. Интерпретация логов OBD: офлайн-оценка (E2, E2b/c, E9)

Блоки I и II разделены на синтетике. В production логи уже собраны под конкретной logging policy; сравнение кандидатов без нового выката — через IPS/SNIPS replay-OPE [24], [26]. Блок не заменяет блок II. Главные диагностические метрики — **acceptance rate**, **ESS** и стабильность SNIPS между seed; сравнение адаптивных политик по максимальному SNIPS при низком acceptance **не интерпретируется** как rollout-ranking. Дополнительно в пайплайне доступен **bootstrap-ДИ для SNIPS** (ресэмплирование строк лога с возвращением, `--bootstrap` в `compare_ab_vs_bandits` / `obd_streaming_ope`); в `run_all` включён для E2/E9/E13.

**Эксперимент E2 (RQ2)** — IPS/SNIPS replay-OPE на `random/all`: 10 000 событий, 80 рук, random logging. Четыре candidate-политики, 20 seed. Acceptance ≈ 1,2% у всех политик (ожидаемо при 80 руках); ESS ≈ 120–124 (табл. 3.12). SNIPS-оценки CTR лежат на уровне 10⁻³ с высокой дисперсией между seed; 38 кликов на 10 000 показов недостаточно для уверенного ranking. OPE демонстрирует пайплайн, но не выбирает «лучший бандит для production».

**Таблица 3.12 — E2: IPS/SNIPS replay-OPE (среднее, 20 seed)**
...
|---|---:|---:|---:|
| thompson_sampling | 0,0121 | 0,0049 | 120,6 |
| fixed_ab | 0,0123 | 0,0029 | 123,4 |
| epsilon_greedy | 0,0124 | 0,0004 | 123,6 |
| ucb1 | 0,0122 | 0,0000 | 122,0 |

**Эксперимент E2b/c (RQ6)** расширяет E2 на четыре подвыборки: `random/all`, `bts/all`, `random/men`, `random/women`, 20 seed. На `bts/all` ESS для `fixed_ab` падает до 58 против 123 на `random/all` — весовая оценка деградирует при adaptive logging (табл. 3.13, рис. 3.2). Максимальный mean SNIPS по подвыборкам приведён **только как иллюстрация** дисперсии оценщика, не как рекомендация политики.

![Рис. 3.2 — ESS matrix OBD](../outputs/figures/fig_3_2_ope_ess_matrix.png)

**Рисунок 3.2 — E2b/c: ESS по политике и подвыборке OBD**

**Таблица 3.13 — E2b/c: максимальный mean SNIPS (иллюстрация; не rollout-вывод)**

| dataset | max mean SNIPS | policy_name | ESS |
|---|---:|---|---:|
| random_all | 0,0049 | thompson_sampling | 120,6 |
| bts_all | 0,0024 | fixed_ab | 57,6 |
| random_men | 0,0147 | ucb1 | 272,0 |
| random_women | 0,0041 | thompson_sampling | 217,8 |

Разведочные batch-прогоны E3 и E8 на OBD (не IPS/SNIPS replay-OPE, без counterfactual для LinUCB) вынесены в **приложение А** — в основной текст не включены.

**Эксперимент E9** масштабирует OPE: E9a — полный `random/all` (1 374 327 событий, 4 768 кликов); E9b — первый миллион `bts/all`, 10 seed, потоковая обработка. ESS вырос примерно в 140 раз (123 → 17 152 для `fixed_ab`), acceptance остаётся ≈ 1,25%. SNIPS между политиками различаются на уровне 10⁻³ при заметной дисперсии между seed (табл. 3.14–3.15); уверенного ranking не получено.

**Таблица 3.14 — E9a: full random/all (среднее ± ст. откл., 10 seed)**

| policy_name | snips_estimate | ESS | acceptance_rate |
|---|---:|---:|---:|
| fixed_ab | 0,00360 ± 0,00022 | 17 152 | 1,25% |
| thompson_sampling | 0,00343 ± 0,00023 | 17 217 | 1,25% |
| epsilon_greedy | 0,00278 ± 0,00021 | 17 603 | 1,28% |
...
**E13 (кратко).** Проекция OBD на пару items 41/50: в 2-ручной постановке acceptance ≈ 49% (против 1,2% в 80-ручной), ESS ≈ 134 при 272 событиях — иллюстрация интерпретируемости метрик, без ranking-выводов.
...
**Propensity и replay-OPE.** Propensity берётся из поля лога (`propensity_score` OBD), а не из candidate-политики — утечки «будущей» информации в веса IPS нет [26]. Candidate-политика **заморожена** при оценке (`freeze_policy=True` в `src/ope/replay.py`): `policy.update()` не вызывается, оценивается фиксированная π. События перетасовываются per seed (`shuffle_events=True`), чтобы снять зависимость от порядка файла. Propensity клипируется снизу (`propensity_floor=0.01`, как в `weighted_inference.py`). Знаменатель IPS — число валидных строк лога с положительной propensity, не общий объём файла. Candidate используется только для сравнения действия с логом (acceptance rate). Сравнение SNIPS адаптивных политик при acceptance ≈ 1/80 **не интерпретируется** как ranking для rollout.
...
**Batch-режим (E3, E8).** Помечен как невалидный для inference и IPS/SNIPS replay-OPE (приложение А); в основные выводы не входит.
...
**Данные и масштаб.** Учебная подвыборка OBD (10k событий, ~40 кликов) не обеспечивает мощности для ranking 80 алгоритмов; даже полный random/all (1,37M событий, ~4,8k кликов) не стабилизировал уверенный ranking по SNIPS. Pairwise-проекция E13 (272 события) интерпретируема, но не заменяет отдельный A/B.

**Методология OPE.** Использован SNIPS replay с замороженной candidate-политикой, не DR [27]; batch E3/E8 — приложение А. Контекст в OBD строится MD5-хэшированием признаков в `scripts/prepare_open_bandit.py` — это не полноценная contextual OPE для LinUCB на random-логах. Разведочный replay-OPE LinUCB на `random/women` (10k событий, 20 seed) дал SNIPS ≈ 0 при ESS ≈ 225 и не включён в табл. 3.12–3.13: при хэш-контексте и низком acceptance оценка не даёт содержательного сравнения с неконтекстными политиками. Выводы о LinUCB опираются на **E8b** (контролируемая синтетика), а не на OPE по OBD. LinUCB не оценивался в streaming E9 на full OBD (P1).

**Статистический вывод.** mSPRT приближённый [19]. IPS Type I ≈ 5% (E11, 100 trials, bootstrap SE); naïve TS ≈ 10–14,5% (E11/E4). E11 и OPE используют bootstrap по строкам лога (`run_all`); adaptively-weighted CI [41] — направление для диплома. **CUPED** [7] реализован как E15 на синтетике; **DR** [27] — в §4.3. Все inference — Monte Carlo на синтетике.
