...
**Дополнение к E1 — сценарии разброса CTR.** Помимо базовой пятирукой среды E1 проверены два крайних случая на той же горизонте T = 5000 и 20 seed: **small_gap** (CTR 4,6–5,4%, шаг ~0,2 п.п.) и **large_gap** (лидер 15% CTR против 1–4% у остальных). При малом разбросе regret у всех политик сходится (21–26), бандит почти не отделяется от `fixed_ab`. При большом разрыве TS резко снижает regret (28,3 против 505,8 у `fixed_ab`): адаптивное перераспределение окупается, когда лучший вариант очевиден (табл. 3.4, рис. 3.4). Это **иллюстрация чувствительности к gap**, а не отдельный статистический вывод: в `synthetic_scenarios` агрегируется только средний regret по seed; bootstrap-ДИ, как в E1, для сценариев не строится.
...
**Статистический вывод.** mSPRT приближённый [19]. IPS Type I ≈ 5% (E11, 100 trials); naïve TS ≈ 10–14,5% (E11/E4). Analytical SE IPS в E11 — row-wise i.i.d.-приближение; для adaptive-логов полная корректность требует bootstrap (`--bootstrap` в `inference_solutions_validity.py`, в `run_all` не включён) или adaptively-weighted CI [41] — направление для диплома (§4.3). CUPED [7] не реализован — см. §4.3. Все inference — Monte Carlo на синтетике.
